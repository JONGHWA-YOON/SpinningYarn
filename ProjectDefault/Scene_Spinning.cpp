...
void Scene_Spinning::PlayTutorial()
{
	D2DEngine::GetInstance()->DrawSpriteAlpha(eObjectDefine::BACK_BLACK, 0, 0, 0.5f);

	if (m_pMouserManager->GetLeftButtonDownState() && !m_clickOnce)
	{
		m_clickOnce = true;
	}

	if (m_pMouserManager->GetLeftButtonUpState() && m_clickOnce)
	{
		m_tutorClickCount++;
		m_clickOnce = false;
	}
	...
	// 카운트를 1로 초기화하면
	// 씬 넘어가자마자 마우스 클릭되면 첫번째 튜토리얼이 빠르게 넘어가서 안보임
	// 그래서 0으로 초기화 후 0일때 1로 바꿔주는 코드 한줄 추가
	//if (m_tutorClickCount == 0)
	//{
	//	m_tutorClickCount = 1;
	//}
	...
	{
		case 1:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_5, 250, 400);
			break;

		case 2:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_6, 450, 200);
			break;
