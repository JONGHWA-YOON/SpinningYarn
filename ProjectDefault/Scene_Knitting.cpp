...
Scene_Knitting::Scene_Knitting(const int sceneNum, const std::string& sceneName, SceneManager* pSceneManager)
	...
	m_tutorClickCount(0), m_clickOnce(true), m_tutorFlag(true)
{
	for (int i = 0; i < 8; i++)
	{
...
void Scene_Knitting::PlayTutorial()
{
	D2DEngine::GetInstance()->DrawSpriteAlpha(eObjectDefine::BACK_BLACK, 0, 0, 0.5f);

	if (MouseManager::GetInstance()->GetLeftButtonDownState() && !m_clickOnce)
	{
		m_clickOnce = true;
	}

	if (MouseManager::GetInstance()->GetLeftButtonUpState() && m_clickOnce)
	{
		m_tutorClickCount++;
		m_clickOnce = false;
	}
	...
	{
		case 1:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_7_1, 300, 100);
			break;

		case 2:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_7_2, 300, 100);
			break;
