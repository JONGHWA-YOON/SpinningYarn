...
Scene_Average::Scene_Average(const int sceneNum, const std::string& sceneName, SceneManager* pSceneManager)
	...
	m_pShop(nullptr), m_pSpiderWeb(nullptr), m_pStatBox(nullptr), m_pStatBox2(nullptr), m_tutorFlag(true),
	m_tutorClickCount(0)
{
	/// 리소스는 여기서 모두 로드한다.
...
void Scene_Average::RenderScene()
{
	...
	if (m_pStageManager->GetStageNum() == 1 && m_tutorFlag)
	{
		PlayTutorial();
	}

	if (m_pStageManager->GetStageNum() == 2 && m_tutorFlag)
	{
		PlayTutorial2();
	}
}
...
//튜토리얼
void Scene_Average::PlayTutorial()
{
	D2DEngine::GetInstance()->DrawSpriteAlpha(eObjectDefine::BACK_BLACK, 0, 0, 0.5f);

	if (m_pMouseManager->GetLeftButtonDownState() && !MouseClickOnce)
	{
		MouseClickOnce = true;
	}

	if (m_pMouseManager->GetLeftButtonUpState() && MouseClickOnce)
	{
		m_tutorClickCount++;
		MouseClickOnce = false;
	}

	switch (m_tutorClickCount)
	{
		case 1:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_1, 950, 3);
			break;

		case 2:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_2, 650, 700);
			break;

		case 3:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_3, 150, 400);
			break;

		case 4:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_4, 1300, 300);
			break;
	...
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_1, 950, 3);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_2, 1300, 50);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_3, 1300, 90);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_4, 1300, 130);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_5, 1300, 170);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_6, 1300, 210);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_7_1, 1300, 250);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_7_2, 1300, 290);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_8, 1300, 330);
	//D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_9, 1300, 370);
}

void Scene_Average::PlayTutorial2()
{
	D2DEngine::GetInstance()->DrawSpriteAlpha(eObjectDefine::BACK_BLACK, 0, 0, 0.5f);
	...
	{
		case 1:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_8, 150, 400);
			break;

		case 2:
			D2DEngine::GetInstance()->DrawSprite(eObjectDefine::Tutor_9, 150, 400);
			break;
